...
type VideoFormat struct {
	...
	// MediaKind, Container and Codec are parsed out of MimeType at read time
	// rather than derived by whoever consumes the record, because -o json is the
	// record and a consumer should not have to re-implement the same three splits.
	...
	//
	// It is media_kind and not kind because a format record also carries the
	// envelope's kind, which is the record kind, and one object cannot have two
	// fields called kind. Doc 03 sections 1 and 7 both claim the name and the
	// envelope wins, since every record has one and only a format has this.
...
type CaptionTrack struct {
	...
	// TrackKind is "asr" on an auto-generated track and empty on a human one,
	// which is why IsAutoGenerated exists: an empty string is not an answer to a
	// question.
	//
	// It is track_kind rather than kind for the reason VideoFormat.MediaKind is
	// media_kind: a caption_track record carries the envelope's kind as well, and
	// the envelope's is the one every record has.
